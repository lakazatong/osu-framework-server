...
namespace osu.Framework.Tests.Visual.UserInterface
{
    public partial class TestSceneContextMenu : ManualInputManagerTestScene
    {
        ...
        {
            base.Content.Add(
                contextMenuContainer = new TestContextMenuContainer { RelativeSizeAxes = Axes.Both }
            );
        }
        ...
        public void TestNestedMenuTransferredWithFadeOut()
        {
                ...
                () =>
                {
                    Child = fadingMenuContainer = new TestContextMenuContainerWithFade
                    {
                        ...
                        Child = new FillFlowContainer
                        {
                            ...
                            Children = new[]
                            {
                                box1 = new BoxWithNestedContextMenuItems
                                {
                                    Size = new Vector2(100),
                                },
                                box2 = new BoxWithNestedContextMenuItems
                                {
                                    Size = new Vector2(100),
                                },
                            },
            ...
            AddStep(
                ...
                () =>
                    InputManager.MoveMouseTo(
                        fadingMenuContainer.ChildrenOfType<Menu.DrawableMenuItem>().First()
                    )
            );
            ...
            AddAssert(
                "submenu opened and visible",
                () =>
                {
                    var targetItem = fadingMenuContainer
                        .ChildrenOfType<Menu.DrawableMenuItem>()
                        .First();
                    var subMenu = fadingMenuContainer.ChildrenOfType<Menu>().Last();

                    return subMenu.State == MenuState.Open
                        && subMenu.IsPresent
                        && !subMenu.IsMaskedAway
                        && subMenu.ScreenSpaceDrawQuad.TopLeft.X
                            > targetItem.ScreenSpaceDrawQuad.TopLeft.X;
                }
            );
        }
        ...
        public void TestHideWhileScrolledAndShow()
        {
            ...
            clickBoxStep(() => box);
            AddAssert(
                "menu has correct size",
                () => contextMenuContainer.CurrentMenu.DrawSize.Y > 10
            );
        }
        ...
        private void addBoxStep(Action<Drawable> boxFunc, int actionCount) =>
            addBoxStep(boxFunc, Enumerable.Repeat(() => { }, actionCount).ToArray());
        ...
        private void assertMenuState(bool opened) =>
            AddAssert(
                $"menu {(opened ? "opened" : "closed")}",
                () => (contextMenuContainer.CurrentMenu?.State == MenuState.Open) == opened
            );

        private void assertMenuInCentre(Func<Drawable> getBoxFunc) =>
            AddAssert(
                "menu in centre of box",
                () =>
                    Precision.AlmostEquals(
                        contextMenuContainer.CurrentMenu.ScreenSpaceDrawQuad.TopLeft,
                        getBoxFunc().ScreenSpaceDrawQuad.Centre
                    )
            );

        private void assertMenuOnScreen(bool expected) =>
            AddAssert(
                $"menu {(expected ? "on" : "off")} screen",
                () =>
                {
                    ...
                    bool result =
                        inputQuad.Contains(menuQuad.TopLeft + new Vector2(1, 1))
        ...
        private void assertMenuItems(int expectedCount) =>
            AddAssert(
                $"menu contains {expectedCount} item(s)",
                () => contextMenuContainer.CurrentMenu.Items.Count == expectedCount
            );
        ...
        private partial class BoxWithContextMenu : Box, IHasContextMenu
        {
            ...
            public MenuItem[] ContextMenuItems =>
                actions?.Select((a, i) => new MenuItem($"Item {i}", a)).ToArray();
        }

        private partial class BoxWithNestedContextMenuItems : Box, IHasContextMenu
        {
            public MenuItem[] ContextMenuItems =>
                new[] { new MenuItem("First") { Items = new[] { new MenuItem("Second") } } };
        }
