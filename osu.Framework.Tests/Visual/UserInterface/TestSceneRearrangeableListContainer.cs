    ...
    public partial class TestSceneRearrangeableListContainer : ManualInputManagerTestScene
    {
        ...
        public void TestAddItem()
        {
            for (int i = 0; i < 5; i++)
            {
                ...
                AddAssert(
                    $"last item is \"{i}\"",
                    () => list.ChildrenOfType<RearrangeableListItem<int>>().Last().Model == localI
                );
            }
        }
        ...
        public void TestBindBeforeLoad()
        {
            AddStep(
                "create list",
                () => list = new TestRearrangeableList { RelativeSizeAxes = Axes.Both }
            );
            AddStep(
                "bind list to items",
                () => list.Items.BindTo(new BindableList<int>(new[] { 1, 2, 3 }))
            );
        ...
        public void TestRemoveItem()
        {
            ...
            AddStep(
                "get item references",
                () => items = new List<Drawable>(list.ItemMap.Values.ToList())
            );
            ...
            for (int i = 0; i < item_count; i++)
            {
                ...
                AddAssert(
                    $"first item is not \"{i}\"",
                    () =>
                        list.ChildrenOfType<RearrangeableListItem<int>>().FirstOrDefault()?.Model
                        != localI
                );
            }

            AddUntilStep(
                "removed items were disposed",
                () => items.Count(i => i.IsDisposed) == item_count
            );
        }
        ...
        public void TestClearItems()
        {
            ...
            AddAssert(
                "no items contained",
                () => !list.ChildrenOfType<RearrangeableListItem<string>>().Any()
            );
        }
        ...
        public void TestRemoveAfterDragScrollThenTryRearrange()
        {
            ...
            AddStep(
                "move the mouse",
                () => InputManager.MoveMouseTo(getItem(0), new Vector2(0, 30))
            );
            ...
            AddStep(
                "move the mouse",
                () => InputManager.MoveMouseTo(getDragger(4), new Vector2(0, 30))
            );
            AddStep("end the drag", () => InputManager.ReleaseButton(MouseButton.Left));
        }
        ...
        public void TestScrolledWhenDraggedToBoundaries()
        {
            ...
            AddStep(
                "drag to 99",
                () => InputManager.MoveMouseTo(getDragger(99), new Vector2(0, 1))
            );
            ...
            AddStep(
                ...
                () =>
                    InputManager.MoveMouseTo(
                        list.ToScreenSpace(list.LayoutRectangle.BottomLeft) + new Vector2(0, 10)
                    )
            );
        ...
        public void TestRearrangeWhileRemovingItems()
        {
            ...
            AddStep(
                ...
                () =>
                    InputManager.MoveMouseTo(
                        list.ToScreenSpace(list.LayoutRectangle.BottomLeft) + new Vector2(0, 20)
                    )
            );
        ...
        public void TestRemoveDuringLoadAndReAdd()
        {
            ...
            AddStep(
                "create list",
                () => Child = delayedList = new TestDelayedLoadRearrangeableList()
            );
            ...
            AddUntilStep(
                "only one item",
                () => delayedList.ChildrenOfType<BasicRearrangeableListItem<int>>().Count() == 1
            );
        }
        ...
        public void TestDragSynchronisation()
        {
            ...
            AddStep(
                "move the mouse",
                () => InputManager.MoveMouseTo(getDragger(0), new Vector2(0, 80))
            );
            AddStep("end the drag", () => InputManager.ReleaseButton(MouseButton.Left));
            ...
            AddAssert(
                ...
                {
                    var item =
                        (BasicRearrangeableListItem<int>)
                            another.ListContainer.FlowingChildren.Last();
                    return item.Model == 0;
                }
            );
        }
        ...
        public void TestReplaceOnlyAppliesDifferences()
        {
            ...
            AddStep(
                "replace {[]} with [1]",
                () => list.Items.ReplaceRange(0, list.Items.Count, [1])
            );
            assertLoadCount(1, 1);
            ...
            AddStep(
                "add {1, []} with [2, 3, 4]",
                () => list.Items.ReplaceRange(list.Items.Count, 0, [2, 3, 4])
            );
            assertLoadCount(2, 1);
            assertLoadCount(3, 1);
            assertLoadCount(4, 1);
            ...
            AddStep(
                "replace {1, [2, 3], 4} with [3, 5]",
                () => list.Items.ReplaceRange(1, 2, [3, 5])
            );
            assertLoadCount(3, 1);
            assertLoadCount(5, 1);

            AddStep(
                "replace {1, [3, 5], 4} with [2, 3]",
                () => list.Items.ReplaceRange(1, 2, [2, 3])
            );
            assertLoadCount(2, 2);
            assertLoadCount(3, 1);
            ...
            AddStep(
                "replace {[1]} with []",
                () => list.Items.ReplaceRange(0, list.Items.Count, [])
            );
            assertLoadCount(1, 1);

            AddStep(
                "replace {[]} with [0, 1, 2, 3, 4, 5, 6]",
                () => list.Items.ReplaceRange(0, list.Items.Count, [0, 1, 2, 3, 4, 5, 6])
            );
            assertLoadCount(0, 1);
            assertLoadCount(1, 2);
            assertLoadCount(2, 3);
            ...
            void assertLoadCount(int item, int expectedTimesLoaded)
            {
                AddUntilStep(
                    ...
                    () => list.ItemMap.Values.All(i => i.IsLoaded)
                );
                AddAssert(
                    $"item {item} loaded {expectedTimesLoaded} times",
                    () => list.LoadCount[item],
        ...
        private void addDragSteps(int from, int to, int[] expectedSequence)
        {
            ...
            AddStep(
                ...
                () =>
                {
                    ...
                    InputManager.MoveMouseTo(
                        getDragger(to),
                        fromDragger.ScreenSpaceDrawQuad.TopLeft.Y
                        < toDragger.ScreenSpaceDrawQuad.TopLeft.Y
                            ? new Vector2(0, 1)
                            : new Vector2(0, -1)
                    );
                }
        ...
        {
            AddAssert(
                $"sequence is {string.Join(", ", sequence)}",
        ...
        private BasicRearrangeableListItem<int>.Button getDragger(int index) =>
            list.ChildrenOfType<BasicRearrangeableListItem<int>>()
                .First(i => i.Model == index)
                .ChildrenOfType<BasicRearrangeableListItem<int>.Button>()
                .First();
        ...
        private partial class TestRearrangeableList : BasicRearrangeableListContainer<int>
        {
            ...
            public new FillFlowContainer<RearrangeableListItem<int>> ListContainer =>
                base.ListContainer;

            public void ScrollTo(int item) =>
                ScrollContainer.ScrollTo(
                    this.ChildrenOfType<BasicRearrangeableListItem<int>>()
                        .First(i => i.Model == item),
                    false
                );
        }

        private partial class TestDelayedLoadRearrangeableList
            : BasicRearrangeableListContainer<int>
        {
            ...
            protected override BasicRearrangeableListItem<int> CreateBasicItem(int item) =>
                new TestRearrangeableListItem(item, AllowLoad);
        ...
        private partial class TestLoadCountingList : BasicRearrangeableListContainer<int>
        {
            ...
            protected override BasicRearrangeableListItem<int> CreateBasicItem(int item) =>
                new TestRearrangeableListItem(
                    item,
                    () => LoadCount[item] = LoadCount.GetValueOrDefault(item) + 1
                );
