    ...
    public partial class Path
    {
        private class PathDrawNode : DrawNode
        {
            ...
            protected override void Draw(IRenderer renderer)
            {
                ...
                triangleBatch ??= renderer.CreateLinearBatch<TexturedVertex3D>(
                    max_res * 200 * 3,
                    10,
            ...
            private Vector2 relativePosition(Vector2 localPos) =>
                Vector2.Divide(localPos, drawSize);

            private Color4 colourAt(Vector2 localPos) =>
                DrawColourInfo.Colour.TryExtractSingleColour(out SRGBColour colour)
                    ? colour.SRGB
                    : DrawColourInfo.Colour.Interpolate(relativePosition(localPos)).SRGB;

            private void addSegmentQuads(
                Line segment,
                Line segmentLeft,
                Line segmentRight,
                RectangleF texRect
            )
            {
                ...
                Vector3 firstMiddlePoint = new Vector3(
            ...
            private void addSegmentCaps(
                float thetaDiff,
                Line segmentLeft,
                Line segmentRight,
                Line prevSegmentLeft,
                Line prevSegmentRight,
                RectangleF texRect
            )
            {
                Debug.Assert(triangleBatch != null);
                ...
                Vector2 current =
                    thetaDiff > 0f ? prevSegmentRight.EndPoint : prevSegmentLeft.EndPoint;
                Vector2 end = thetaDiff > 0f ? segmentRight.StartPoint : segmentLeft.StartPoint;

                Line start =
                    thetaDiff > 0f
                        ? new Line(prevSegmentLeft.EndPoint, prevSegmentRight.EndPoint)
                        : new Line(prevSegmentRight.EndPoint, prevSegmentLeft.EndPoint);
                ...
                for (int i = 1; i <= stepCount; i++)
                {
                    ...
                    current =
                        i < stepCount
                            ? origin + pointOnCircle(theta0 + i * thetaStep) * radius
                            : end;
            ...
            private void updateVertexBuffer()
            {
                ...
                RectangleF texRect = texture.GetTextureRect(
                    new RectangleF(0.5f, 0.5f, texture.Width - 1, texture.Height - 1)
                );
                ...
                for (int i = 0; i < segments.Count; i++)
                {
                    ...
                    Line currSegmentLeft = new Line(
                        currSegment.StartPoint + ortho * radius,
                        currSegment.EndPoint + ortho * radius
                    );
                    Line currSegmentRight = new Line(
                        currSegment.StartPoint - ortho * radius,
                        currSegment.EndPoint - ortho * radius
                    );
                    ...
                    if (i == 0)
                    {
                        ...
                        Line flippedLeft = new Line(
                            currSegmentRight.EndPoint,
                            currSegmentRight.StartPoint
                        );
                        Line flippedRight = new Line(
                            currSegmentLeft.EndPoint,
                            currSegmentLeft.StartPoint
                        );
