...
namespace osu.Framework.Tests.Visual.Sprites
{
    public partial class TestSceneSmoothedEdges : GridTestScene
    {
            ...
            : base(2, 3)
        {
            ...
            AddAssert(
                "(1, 0) has inflation only in y",
                () => boxes[1].InflationAmount.X == 0 && boxes[1].InflationAmount.Y > 0
            );
            AddAssert(
                "(2, 0) has inflation only in x",
                () => boxes[2].InflationAmount.X > 0 && boxes[2].InflationAmount.Y == 0
            );
            AddAssert(
                "(0, 1) has inflation in x and y",
                () => boxes[3].InflationAmount.X > 0 && boxes[3].InflationAmount.Y > 0
            );
            AddAssert(
                "(1, 1) has inflation in x and y",
                () => boxes[4].InflationAmount.X > 0 && boxes[4].InflationAmount.Y > 0
            );
        }
