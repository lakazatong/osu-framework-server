...
namespace osu.Framework.Testing
{
    ...
    public abstract partial class TestScene : Container
    {
        ...
        public override void Add(Drawable drawable)
        {
            ...
            if (drawable is Game)
                throw new InvalidOperationException(
                    $"Use {nameof(AddGame)} when testing a game instance."
                );
        ...
        {
            throw new InvalidOperationException(
                $"Modifying {nameof(InternalChildren)} will cause critical failure. Use {nameof(Add)} instead."
            );
        }

        protected internal override void ClearInternal(bool disposeChildren = true) =>
            throw new InvalidOperationException(
                $"Modifying {nameof(InternalChildren)} will cause critical failure. Use {nameof(Clear)} instead."
            );

        protected internal override bool RemoveInternal(
            Drawable drawable,
            bool disposeImmediately
        ) =>
            throw new InvalidOperationException(
                $"Modifying {nameof(InternalChildren)} will cause critical failure. Use {nameof(Remove)} instead."
            );
        ...
        public void RunAllSteps(
            ...
            Action<StepButton, Exception> onError = null,
            Func<StepButton, bool> stopCondition = null,
            StepButton startFromStep = null
        )
        {
            ...
            Schedule(() =>
            {
                ...
                actionIndex =
                    startFromStep != null ? StepsContainer.IndexOf(startFromStep) + 1 : -1;
                actionRepetition = 0;
                runNextStep(onCompletion, onError, stopCondition);
            });
        }

        private StepButton loadableStep =>
            actionIndex >= 0
                ? StepsContainer.Children.ElementAtOrDefault(actionIndex) as StepButton
                : null;

        private void runNextStep(
            Action onCompletion,
            Action<StepButton, Exception> onError,
            Func<StepButton, bool> stopCondition
        )
        {
            try
            {
                ...
                new LabelStep
                {
                    ...
                    Action = step =>
                    {
                        ...
                        if (step.IsHovered)
                            RunAllSteps(
                                startFromStep: step,
                                stopCondition: s => s is LabelStep,
                                onError: (s, e) => Logger.Error(e, $"Step {s} triggered error")
                            );
                    },
                }
            );
        ...
        protected void AddUntilStep(
            [CanBeNull] string description,
            [NotNull] Func<bool> waitUntilTrueDelegate
        )
        {
            AddStep(
                new UntilStepButton
        ...
        protected void AddUntilStep<T>(
            [CanBeNull] string description,
            [NotNull] ActualValueDelegate<T> actualValue,
            [NotNull] Func<IResolveConstraint> constraint
        )
        {
        ...
        protected void AddSliderStep<T>(
            [NotNull] string description,
            T min,
            T max,
            T start,
            [NotNull] Action<T> valueChanged
        )
            where T : struct, INumber<T>, IMinMaxValue<T>
        {
            ...
            {
                StepsContainer.Add(
                    new StepSlider<T>(description, min, max, start) { ValueChanged = valueChanged }
                );
            });
        }

        protected void AddAssert(
            [NotNull] string description,
            [NotNull] Func<bool> assert,
        ...
        internal void RunSetUpSteps()
        {
            ...
            foreach (
                var method in ReflectionUtils.GetMethodsWithAttribute(
                    GetType(),
                    typeof(SetUpStepsAttribute),
                    true
                )
            )
        ...
        private class UseTestSceneRunnerAttribute : TestActionAttribute
        {
            public override void BeforeTest(ITest test)
            {
                ...
                bool hasSoloAttribute =
                    test.Method?.MethodInfo.CustomAttributes.Any(a =>
                        a.AttributeType == typeof(SoloAttribute)
                    ) == true;
                if (DebugUtils.IsNUnitRunning && hasSoloAttribute)
                    throw new InvalidOperationException(
                        $"{nameof(SoloAttribute)} should not be specified on tests running under NUnit."
                    );
                ...
                foreach (var thread in testScene.host.Threads)
                {
                    thread.Scheduler.Add(() =>
                    {
                        TestExecutionContext.CurrentContext.CurrentResult =
                            executionContext.CurrentResult;
                        TestExecutionContext.CurrentContext.CurrentTest =
                            executionContext.CurrentTest;
                        TestExecutionContext.CurrentContext.CurrentCulture =
                            executionContext.CurrentCulture;
                        TestExecutionContext.CurrentContext.CurrentPrincipal =
                            executionContext.CurrentPrincipal;
                        TestExecutionContext.CurrentContext.CurrentRepeatCount =
                            executionContext.CurrentRepeatCount;
