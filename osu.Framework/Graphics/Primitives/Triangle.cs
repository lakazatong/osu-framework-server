...
namespace osu.Framework.Graphics.Primitives
{
    ...
    public readonly struct Triangle : IConvexPolygon, IEquatable<Triangle>
    {
        ...
        public ReadOnlySpan<Vector2> GetVertices() =>
            MemoryMarshal.CreateReadOnlySpan(ref Unsafe.AsRef(in P0), 3);

        public bool Equals(Triangle other) => P0 == other.P0 && P1 == other.P1 && P2 == other.P2;
        ...
        public bool Contains(Vector2 pos)
        {
            ...
            float s =
                (P0.Y * P2.X - P0.X * P2.Y + (P2.Y - P0.Y) * pos.X + (P0.X - P2.X) * pos.Y) / area2;
            if (s < 0)
                return false;

            float t =
                (P0.X * P1.Y - P0.Y * P1.X + (P0.Y - P1.Y) * pos.X + (P1.X - P0.X) * pos.Y) / area2;
            if (t < 0 || s + t > 1)
                return false;
