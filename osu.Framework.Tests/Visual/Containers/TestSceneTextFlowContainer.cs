...
namespace osu.Framework.Tests.Visual.Containers
{
    public partial class TestSceneTextFlowContainer : FrameworkTestScene
    {
        private const string default_text =
            "Default text which is long enough such that it will break a line\n\nnewline";
            ...
            AddAssert(
                ...
                () =>
                {
                    string result = string.Concat(
                        textContainer
                            .Children.OrderBy(c => c.ScreenSpaceDrawQuad.TopLeft.Y)
                            .ThenBy(c =>
                                c is TextFlowContainer.NewLineContainer
                                    ? 0
                                    : c.ScreenSpaceDrawQuad.TopLeft.X
                            )
        ...
        public void TestSetText()
        {
            ...
            AddAssert(
                "text flow has 2 sprite texts",
                () => textContainer.ChildrenOfType<SpriteText>().Count() == 2
            );
        ...
        public void TestPartManagement()
        {
            ...
            AddStep(
                "set text colour to red manually",
                () => part.Drawables.ForEach(p => p.Colour = Colour4.Red)
            );
        ...
        public void TestWordSplittingEdgeCases()
        {
            ...
            AddStep(
                ...
                () =>
                    textContainer.Text =
                        "https://osu.ppy.sh/home/news/2024-03-27-osutaiko-world-cup-2024-round-of-32-recap"
            );
            AddStep(
                "set cjk text",
                () =>
                    textContainer.Text =
                        "日本の桜は世界中から観光客を引きつけています。寿司は美味しい伝統的な日本食です。東京タワーは景色が美しいです。速い新幹線は、便利な交通手段です。富士山は、その美しさと完全な形状で知られています。日本文化は、優雅さと繊細さを象徴しています。抹茶は特別な日本の茶です。着物は、伝統的な日本の衣装で、特別な場面でよく着用されます。"
            );
        }
        ...
        public void TestSizing()
        {
            ...
            AddStep(
                "set autosize width with right anchored text",
                () =>
                {
                    topLevelContainer.RelativeSizeAxes = textContainer.RelativeSizeAxes = Axes.None;
                    topLevelContainer.AutoSizeAxes = textContainer.AutoSizeAxes = Axes.Both;
        ...
        public void TestSetTextRepeatedly()
        {
            ...
            AddStep(
                "set text repeatedly",
                () =>
                    repeat = Scheduler.AddDelayed(
                        () =>
                        {
                            textContainer.Clear();
                            textContainer.AddParagraph(
                                "first paragraph lorem ipsum dolor sit amet and whatever else is needed to break a line"
                            );
                            textContainer.AddParagraph(string.Empty);
                            textContainer.AddParagraph(
                                "second paragraph lorem ipsum dolor sit amet and whatever else is needed to break a line"
                            );
                        },
                        50,
                        true
        ...
        {
            AddStep(
                "set text to break at last word of paragraph",
                () =>
                {
                    textContainer.Clear();
                    ...
                    textContainer.AddParagraph(string.Empty);
                    textContainer.AddParagraph(
                        "second paragraph lorem ipsum dolor sit ametttttttttttttttttt"
                    );
        ...
        private void assertSpriteTextCount(int count) =>
            AddAssert(
                $"text flow has {count} sprite texts",
                () => textContainer.ChildrenOfType<SpriteText>().Count() == count
            );

        private void assertTotalChildCount(int count) =>
            AddAssert(
                $"text flow has {count} children",
                () => textContainer.Children.Count() == count
            );
    }
}
