...
namespace osu.Framework.SourceGeneration.Generators.Dependencies.Emitters
{
    public class DependenciesFileEmitter : IncrementalSourceEmitter
    {
        ...
        public const string REGISTER_FOR_DEPENDENCY_ACTIVATION_METHOD_NAME =
                        ...
                        SyntaxFactory.SingletonSeparatedList<BaseTypeSyntax>(
                            SyntaxFactory.SimpleBaseType(
                                SyntaxFactory.ParseTypeName(
                                    "global::osu.Framework.Allocation.ISourceGeneratedDependencyActivator"
                                )
                            )
                        )
                    )
                                ...
                                SyntaxFactory.ParameterList(
                                    SyntaxFactory.SingletonSeparatedList(
                                        SyntaxFactory
                                            ...
                                            .WithType(
                                                SyntaxFactory.ParseTypeName(
                                                    "global::osu.Framework.Allocation.IDependencyActivatorRegistry"
                                                )
                                            )
                                    )
                                )
                            ...
                            SyntaxFactory.SeparatedList(
                                new[]
                                {
                                    SyntaxFactory.Argument(
                                        SyntaxFactory.TypeOfExpression(
                                            SyntaxFactory.ParseTypeName(
                                                Target.GlobalPrefixedTypeName
                                            )
                                        )
                                    ),
                                    SyntaxFactory.Argument(emitInjectDependenciesDelegate()),
                                    SyntaxFactory.Argument(emitCacheDependenciesDelegate()),
                                }
                            )
                        )
                    )
        ...
        private ExpressionSyntax emitInjectDependenciesDelegate()
        {
            ...
            return SyntaxFactory
                ...
                .WithBlock(
                    SyntaxFactory.Block(
                        Target
                            .ResolvedMembers.Select(m =>
                                (IStatementEmitter)new ResolvedMemberEmitter(this, m)
                            )
                            .Concat(
                                Target.DependencyLoaderMembers.Select(
                                    m => new BackgroundDependencyLoaderEmitter(this, m)
                                )
                            )
                            .SelectMany(e => e.Emit())
                    )
        ...
        {
            if (
                Target.CachedMembers.Count == 0
                && Target.CachedInterfaces.Count == 0
                && Target.CachedClasses.Count == 0
            )
                return SyntaxFactory.LiteralExpression(SyntaxKind.NullLiteralExpression);
            ...
            return SyntaxFactory
                ...
                .WithBlock(
                    SyntaxFactory.Block(
                        Target
                            .CachedMembers.Select(m =>
                                (IStatementEmitter)new CachedMemberEmitter(this, m)
                            )
                            .Concat(
                                Target.CachedClasses.Select(m => new CachedClassEmitter(this, m))
                            )
                            .Concat(
                                Target.CachedInterfaces.Select(m => new CachedInterfaceEmitter(
                                    this,
                                    m
                                ))
                            )
                                        ...
                                        SyntaxFactory.EqualsValueClause(
                                            SyntaxFactory
                                                .ObjectCreationExpression(
                                                    SyntaxFactory.ParseTypeName(
                                                        "global::osu.Framework.Allocation.DependencyContainer"
                                                    )
                                                )
                                                .WithArgumentList(
                                                    SyntaxFactory.ArgumentList(
            ...
            static StatementSyntax createEpilogue() =>
                SyntaxFactory.ReturnStatement(
                    SyntaxFactory.IdentifierName(LOCAL_DEPENDENCIES_VAR_NAME)
                );
        }
    }
}
