...
namespace osu.Framework.Android.Graphics.Video
{
    ...
    public unsafe class AndroidVideoDecoder : VideoDecoder
    {
        ...
        [DllImport(lib_avformat)]
        private static extern int av_seek_frame(
            AVFormatContext* s,
            int stream_index,
            long timestamp,
            int flags
        ...
        [DllImport(lib_avutil)]
        private static extern int av_hwdevice_ctx_create(
        ...
        [DllImport(lib_avcodec)]
        private static extern int avcodec_parameters_to_context(
            AVCodecContext* codec,
            AVCodecParameters* par
        );

        [DllImport(lib_avcodec)]
        private static extern int avcodec_open2(
        ...
        [DllImport(lib_avformat)]
        private static extern int avformat_find_stream_info(
            AVFormatContext* ic,
            AVDictionary** options
        );
        ...
        [DllImport(lib_avformat)]
        private static extern int av_find_best_stream(
            AVFormatContext* ic,
            AVMediaType type,
        ...
        [DllImport(lib_avformat)]
        private static extern AVIOContext* avio_alloc_context(
            byte* buffer,
            int buffer_size,
            int write_flag,
            void* opaque,
        ...
        [DllImport(lib_swscale)]
        private static extern SwsContext* sws_getCachedContext(
            SwsContext* context,
            int srcW,
            int srcH,
        ...
        [DllImport(lib_swscale)]
        private static extern int sws_scale(
            SwsContext* c,
            byte*[] srcSlice,
            int[] srcStride,
            int srcSliceY,
            ...
            : base(renderer, videoStream)
        {
            ...
            int result = av_jni_set_java_vm(
                JniEnvironment.Runtime.InvocationPointer.ToPointer(),
                null
            );
            if (result < 0)
                throw new InvalidOperationException(
                    $"Couldn't pass Java VM handle to FFmpeg: ${result}"
                );
        }
        ...
        )
        {
            if (targetHwDecoders.HasFlagFast(HardwareVideoDecoder.MediaCodec))
            {
                ...
                switch (formatName)
                {
                    ...
                    {
                        Logger.Log(
                            $"Disabling HW decoding for this video because of unsupported input format: ${formatName}"
                        );
