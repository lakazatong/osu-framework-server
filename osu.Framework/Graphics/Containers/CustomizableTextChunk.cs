        ...
        where TSpriteText : SpriteText, new()
    {
        ...
        )
        {
            ...
            while (index < str.Length)
            {
                ...
                int nextPlaceholderIndex = str.IndexOf(
                    CustomizableTextContainer.UNESCAPED_LEFT,
                    index,
                ...
                while (
                    ...
                    && str.IndexOf(
                        CustomizableTextContainer.ESCAPED_LEFT,
                        nextPlaceholderIndex,
                        StringComparison.Ordinal
                    ) == nextPlaceholderIndex
                )
                    nextPlaceholderIndex = str.IndexOf(
                        CustomizableTextContainer.UNESCAPED_LEFT,
                        nextPlaceholderIndex + 2,
                        StringComparison.Ordinal
                    );
                ...
                {
                    int placeholderEnd = str.IndexOf(
                    ...
                    while (
                        placeholderEnd != -1
                        && str.IndexOf(
                            CustomizableTextContainer.ESCAPED_RIGHT,
                            placeholderEnd,
                            StringComparison.InvariantCulture
                        ) == placeholderEnd
                    )
                        placeholderEnd = str.IndexOf(
                            CustomizableTextContainer.UNESCAPED_RIGHT,
                            placeholderEnd + 2,
                            StringComparison.Ordinal
                        );
                    ...
                    if (placeholderEnd != -1)
                    {
                        ...
                        string placeholderStr = str.AsSpan(
                                nextPlaceholderIndex + 1,
                                placeholderEnd - nextPlaceholderIndex - 1
                            )
                            .Trim()
                            .ToString();
                        ...
                        if (parensOpen != -1)
                        {
                            ...
                            if (parensClose != -1)
                                paramStr = placeholderStr
                                    .AsSpan(parensOpen + 1, parensClose - parensOpen - 1)
                                    .Trim()
                                    .ToString();
                            else
                                throw new ArgumentException(
                                    $"Missing ) in placeholder {placeholderStr}."
                                );
                        }
                        ...
                        if (int.TryParse(placeholderStr, out int placeholderIndex))
                        {
                            if (placeholderIndex < 0)
                                throw new ArgumentException(
                                    $"Negative placeholder indices are invalid. Index {placeholderIndex} was used."
                                );

                            placeholderDrawable =
                                customizableContainer.Placeholders.ElementAtOrDefault(
                                    placeholderIndex
                                );
                            if (placeholderDrawable == null)
                                throw new ArgumentException(
                                    $"Placeholder with index {placeholderIndex} is null, or {placeholderIndex} is outside the bounds of allowable placeholder indices."
                                );
                        }
                        else
                        {
                            ...
                            else
                            {
                                ...
                                for (int i = 0; i < argStrs.Length; ++i)
                                {
                                    if (!int.TryParse(argStrs[i], out int argVal))
                                        throw new ArgumentException(
                                            $"The argument \"{argStrs[i]}\" in placeholder {placeholderStr} is not an integer."
                                        );
                            ...
                            if (
                                !customizableContainer.TryGetIconFactory(
                                    placeholderName,
                                    out Delegate cb
                                )
                            )
                                throw new ArgumentException(
                                    $"There is no placeholder named {placeholderName}."
                                );
