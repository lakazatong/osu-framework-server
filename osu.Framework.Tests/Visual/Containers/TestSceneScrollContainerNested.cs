...
namespace osu.Framework.Tests.Visual.Containers
{
    public partial class TestSceneScrollContainerNested : ManualInputManagerTestScene
    {
        ...
        private static bool checkScrollCurrent(
            BasicScrollContainer scrolled,
            BasicScrollContainer notScrolled
        ) => notScrolled.Current == 0 && Precision.DefinitelyBigger(scrolled.Current, 0f);
            ...
            AddStep(
                ...
                () =>
                {
                    Child = outerScroll = new BasicScrollContainer(outer)
                    {
                        ...
                        {
                            new Box { Size = new Vector2(500f), Colour = FrameworkColour.Yellow },
        ...
        public void TestDragging(Direction outer, Direction inner)
        {
            ...
            AddAssert(
                "vertical container scrolled only",
                () => checkScrollCurrent(verticalScroll, horizontalScroll)
            );
            AddStep(
            ...
            AddAssert(
                "horizontal container scrolled only",
                () => checkScrollCurrent(horizontalScroll, verticalScroll)
            );
            AddStep(
        ...
        public void TestNonPreciseScrolling(Direction outer, Direction inner, bool holdShift)
        {
            ...
            else
            {
                ...
                AddAssert(
                    "inner container scrolled only",
                    () => checkScrollCurrent(innerScroll, outerScroll)
                );
