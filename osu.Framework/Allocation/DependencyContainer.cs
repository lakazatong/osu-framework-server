...
namespace osu.Framework.Allocation
{
    ...
    public class DependencyContainer : IReadOnlyDependencyContainer
    {
        ...
        public void Cache(object instance) => Cache(instance, default);
        ...
        public void CacheAs<T>(T instance)
            where T : class => CacheAs(instance, default);
        ...
        public void CacheAs<T>(T instance, CacheInfo info)
            where T : class => CacheAs(typeof(T), info, instance, false);
        ...
        public void CacheAs<T>(Type type, T instance)
            where T : class => CacheAs(type, instance, default);
        ...
        public void CacheAs<T>(Type type, T instance, CacheInfo info)
            where T : class => CacheAs(type, info, instance, false);
        ...
        /// </remarks>
        /// <param name="instance">The instance to cache.</param>
        internal void CacheValue(object instance) => CacheValue(instance, default);
        ...
        /// </remarks>
        /// <param name="instance">The instance to cache. Must be or derive from <typeparamref name="T"/>.</param>
        internal void CacheValueAs<T>(T instance) => CacheValueAs(instance, default);
        ...
        internal void CacheAs(Type type, CacheInfo info, object instance, bool allowValueTypes)
        {
            ...
            if (instanceType.IsValueType && !allowValueTypes)
                throw new ArgumentException(
                    $"{instanceType.ReadableName()} must be a class to be cached as a dependency.",
                    nameof(instance)
                );

            if (!info.Type.IsInstanceOfType(instance))
                throw new ArgumentException(
                    $"{instanceType.ReadableName()} must be a subclass of {info.Type.ReadableName()}.",
                    nameof(instance)
                );
    ...
    public class TypeAlreadyCachedException : InvalidOperationException
    {
        public TypeAlreadyCachedException(CacheInfo info)
            : base(
                $"An instance of the member ({info.ToString()}) has already been cached to the dependency container."
            ) { }
    }
}
