...
namespace osu.Framework.Tests.Visual.Input
{
    ...
    {
        private static readonly TouchSource[] touch_sources = (TouchSource[])
            Enum.GetValues(typeof(TouchSource));
        ...
        private float getTouchXPos(TouchSource source) =>
            receptors[(int)source].DrawPosition.X
            + Content.DrawWidth / TouchState.MAX_TOUCH_COUNT / 2;

        private Vector2 getTouchDownPos(TouchSource source) =>
            receptors.ToScreenSpace(new Vector2(getTouchXPos(source), 50f));

        private Vector2 getTouchMovePos(TouchSource source) =>
            receptors.ToScreenSpace(new Vector2(getTouchXPos(source), receptors.DrawHeight / 2f));

        private Vector2 getTouchUpPos(TouchSource source) =>
            receptors.ToScreenSpace(new Vector2(getTouchXPos(source), receptors.DrawHeight - 1f));
        ...
        public void TestTouchInputHandling()
        {
            ...
            AddAssert(
                "received correct event for each receptor",
                () =>
                {
                    foreach (var r in receptors)
                    {
                        ...
                        if (
                            !(
                                r.TouchEvents.TryDequeue(out TouchEvent te)
                                && te is TouchDownEvent touchDown
                            )
                        )
                            return false;
                        ...
                        if (
                            touchDown.ScreenSpaceTouch.Source != r.AssociatedSource
                            || touchDown.ScreenSpaceTouch.Position
                                != getTouchDownPos(r.AssociatedSource)
                            || touchDown.ScreenSpaceTouchDownPosition
                                != getTouchDownPos(r.AssociatedSource)
                        )
                            return false;
                ...
                () =>
                {
                    foreach (var r in receptors)
                    {
                        if (
                            !(
                                r.TouchEvents.TryDequeue(out TouchEvent te)
                                && te is TouchMoveEvent touchMove
                            )
                        )
                            return false;

                        if (
                            touchMove.ScreenSpaceTouch.Source != r.AssociatedSource
                            || touchMove.ScreenSpaceTouch.Position
                                != getTouchMovePos(r.AssociatedSource)
                            || touchMove.ScreenSpaceLastTouchPosition
                                != getTouchDownPos(r.AssociatedSource)
                            || touchMove.ScreenSpaceTouchDownPosition
                                != getTouchDownPos(r.AssociatedSource)
                        )
                            return false;
                ...
                () =>
                {
                    foreach (var r in receptors)
                    {
                        ...
                        if (
                            touchMove.ScreenSpaceTouch.Source != r.AssociatedSource
                            || touchMove.ScreenSpaceTouch.Position
                                != getTouchUpPos(r.AssociatedSource)
                            || touchMove.ScreenSpaceLastTouchPosition
                                != getTouchMovePos(r.AssociatedSource)
                            || touchMove.ScreenSpaceTouchDownPosition
                                != getTouchDownPos(r.AssociatedSource)
            ...
            AddStep(
                "deactivate touches out of receptors",
                () =>
                {
                    foreach (var s in touch_sources)
                ...
                () =>
                {
                    foreach (var r in receptors)
                    {
                        if (
                            !(
                                r.TouchEvents.TryDequeue(out TouchEvent te)
                                && te is TouchUpEvent touchUp
                            )
                        )
                            return false;

                        if (
                            touchUp.ScreenSpaceTouch.Source != r.AssociatedSource
                            || touchUp.ScreenSpaceTouch.Position
                                != getTouchUpPos(r.AssociatedSource)
                            || touchUp.ScreenSpaceTouchDownPosition
                                != getTouchDownPos(r.AssociatedSource)
                        )
                            return false;
            ...
            AddAssert(
                "no mouse input performed",
                () => receptors.All(r => r.MouseEvents.Count == 0)
            );
        }
        ...
        {
            InputReceptor firstReceptor = null,
                lastReceptor = null;

            AddStep(
                "setup receptors to receive left mouse-from-touch",
                () =>
                {
            ...
            AddStep(
                ...
                () =>
                {
                    InputManager.BeginTouch(
                        new Touch(
                            firstReceptor.AssociatedSource,
                            getTouchDownPos(firstReceptor.AssociatedSource)
                        )
                    );
                }
            );
            ...
            AddAssert(
                "received mouse-down event on first",
                () =>
                {
                    // event #1: move mouse to first touch position.
                    if (
                        !(
                            firstReceptor.MouseEvents.TryDequeue(out MouseEvent me1)
                            && me1 is MouseMoveEvent mouseMove
                        )
                    )
                        return false;

                    if (
                        mouseMove.ScreenSpaceMousePosition
                        != getTouchDownPos(firstReceptor.AssociatedSource)
                    )
                        return false;
                    ...
                    if (
                        !(
                            firstReceptor.MouseEvents.TryDequeue(out MouseEvent me2)
                            && me2 is MouseDownEvent mouseDown
                        )
                    )
                        return false;
                    ...
                    if (
                        ...
                        || mouseDown.ScreenSpaceMousePosition
                            != getTouchDownPos(firstReceptor.AssociatedSource)
                        || mouseDown.ScreenSpaceMouseDownPosition
                            != getTouchDownPos(firstReceptor.AssociatedSource)
                    )
                        return false;
            ...
            foreach (var s in touch_sources.Skip(1))
            {
                ...
                AddStep(
                    $"activate {s}",
                    () => InputManager.BeginTouch(touch = new Touch(s, getTouchDownPos(s)))
                );
                AddAssert(
                    "mouse jumped to new touch",
                    () => assertMouseOnTouchChange(touch, null, true)
                );
            }
            ...
            foreach (var s in touch_sources)
            {
                ...
                AddStep(
                    $"move {s} inside area",
                    () => InputManager.MoveTouchTo(touch = new Touch(s, getTouchMovePos(s)))
                );
                AddAssert(
                    "received regular mouse-move event",
                    () =>
                    {
            ...
            foreach (var s in touch_sources)
            {
                ...
                AddStep(
                    $"move {s} outside of area",
                    () => InputManager.MoveTouchTo(touch = new Touch(s, getTouchUpPos(s)))
                );
                AddAssert(
                    "no mouse-move event received",
                    () =>
                    {
                        // ReSharper disable once AccessToModifiedClosure
            ...
            foreach (var s in touch_sources.SkipLast(1))
            {
                ...
                AddAssert(
                    "no mouse event received",
                    () => receptors[(int)s].MouseEvents.Count == 0
                );
            }
            ...
            AddStep(
                ...
                () =>
                {
                    InputManager.EndTouch(
                        new Touch(
                            lastReceptor.AssociatedSource,
                            getTouchUpPos(lastReceptor.AssociatedSource)
                        )
                    );
                }
            );
            ...
            AddAssert(
                "received mouse-up event",
                () =>
                {
                    // First receptor is the one handling the mouse down event, mouse up would be raised to it.
                    if (
                        !(
                            firstReceptor.MouseEvents.TryDequeue(out MouseEvent me)
                            && me is MouseUpEvent mouseUp
                        )
                    )
                        return false;
                    ...
                    if (
                        ...
                        || mouseUp.ScreenSpaceMousePosition
                            != getTouchUpPos(lastReceptor.AssociatedSource)
                        || mouseUp.ScreenSpaceMouseDownPosition
                            != getTouchDownPos(firstReceptor.AssociatedSource)
                    )
                        return false;
            ...
            bool assertMouseOnTouchChange(Touch touch, Vector2? lastPosition, bool expectsMouseMove)
            {
                ...
                if (expectsMouseMove)
                {
                    if (
                        !(
                            receptor.MouseEvents.TryDequeue(out MouseEvent me1)
                            && me1 is MouseMoveEvent mouseMove
                        )
                    )
                        return false;
                    ...
                    if (
                        mouseMove.ScreenSpaceMousePosition != touch.Position
                        || (
                            lastPosition != null
                            && mouseMove.ScreenSpaceLastMousePosition != lastPosition.Value
                        )
                    )
                        return false;
                }
                ...
                if (
                    !(
                        firstReceptor.MouseEvents.TryDequeue(out MouseEvent me2)
                        && me2 is DragEvent mouseDrag
                    )
                )
                    return false;

                if (
                    mouseDrag.Button != MouseButton.Left
                    || mouseDrag.ScreenSpaceMousePosition != touch.Position
                    || (
                        lastPosition != null
                        && mouseDrag.ScreenSpaceLastMousePosition != lastPosition.Value
                    )
                    || mouseDrag.ScreenSpaceMouseDownPosition
                        != getTouchDownPos(firstReceptor.AssociatedSource)
                )
                    return false;
        ...
        public void TestMouseEventFromTouchIndication()
        {
            ...
            AddStep(
                "retrieve primary receptor",
                () => primaryReceptor = receptors[(int)TouchSource.Touch1]
            );
            AddStep(
                "setup receptors to discard mouse-from-touch events",
                () =>
                {
                    primaryReceptor.HandleTouch = _ => false;
                    primaryReceptor.HandleMouse = e =>
                        !(e.CurrentState.Mouse.LastSource is ISourcedFromTouch);
                }
            );

            AddStep(
                "perform input on primary touch",
                () =>
                {
                    InputManager.BeginTouch(
                        new Touch(TouchSource.Touch1, getTouchDownPos(TouchSource.Touch1))
                    );
                    InputManager.MoveTouchTo(
                        new Touch(TouchSource.Touch1, getTouchMovePos(TouchSource.Touch1))
                    );
                    InputManager.EndTouch(
                        new Touch(TouchSource.Touch1, getTouchUpPos(TouchSource.Touch1))
                    );
                }
            );
            ...
            AddAssert(
                "all mouse events received",
                () =>
                {
                    // mouse moved.
                    if (
                        !(
                            primaryReceptor.MouseEvents.TryDequeue(out var me1)
                            && me1 is MouseMoveEvent
                        )
                    )
                        return false;
                    ...
                    if (
                        !(
                            primaryReceptor.MouseEvents.TryDequeue(out var me2)
                            && me2 is MouseDownEvent
                        )
                    )
                        return false;
                    ...
                    if (
                        !(
                            primaryReceptor.MouseEvents.TryDequeue(out var me3)
                            && me3 is MouseMoveEvent
                        )
                    )
                        return false;
                    ...
                    if (
                        !(
                            primaryReceptor.MouseEvents.TryDequeue(out var me5)
                            && me5 is MouseUpEvent
                        )
                    )
                        return false;
        ...
        public void TestMouseStillReleasedOnHierarchyInterference()
        {
            ...
            AddStep(
                "setup handlers to receive left mouse-from-touch",
                () =>
                {
                    InputManager.RightClickFromLongTouch = false;
                    primaryReceptor.HandleTouch = _ => false;
            ...
            AddAssert(
                "primary receptor received mouse",
                () =>
                {
            ...
            AddStep(
                ...
                () =>
                    primaryReceptor.Add(
                        childReceptor = new InputReceptor(TouchSource.Touch1)
                        {
                            RelativeSizeAxes = Axes.Both,
                            HandleTouch = _ => true,
                        }
            ...
            AddAssert(
                "child receptor received nothing",
                () => childReceptor.TouchEvents.Count == 0 && childReceptor.MouseEvents.Count == 0
            );
        }
        ...
        public void TestHoldTouchToRightClick()
        {
            ...
            AddStep(
                "setup handlers to receive mouse-from-touch",
                () =>
                {
                    primaryReceptor.HandleTouch = _ => false;
                }
            ...
            AddAssert(
                ...
                () =>
                {
                    bool event1 = primaryReceptor.MouseEvents.Dequeue() is MouseMoveEvent;
                    bool event2 =
                        primaryReceptor.MouseEvents.Dequeue() is MouseDownEvent down
            ...
            AddAssert(
                ...
                () =>
                {
                    ...
                    bool event2 =
                        primaryReceptor.MouseEvents.Dequeue() is MouseUpEvent up
            ...
            AddAssert(
                "received move and left drag",
                () =>
                {
                    bool event1 = primaryReceptor.MouseEvents.Dequeue() is MouseMoveEvent;
                    bool event2 =
                        primaryReceptor.MouseEvents.Dequeue() is DragEvent drag
        ...
        public void TestHoldTouchThenKeepHoldingAfterRightClick()
        {
            ...
            AddStep(
                ...
                () =>
                {
                    primaryReceptor.HandleTouch = _ => false;
                    primaryReceptor.HandleMouse = e =>
                        e is MouseButtonEvent button && button.Button == MouseButton.Right;
                }
            );
            ...
            AddStep(
                ...
                () =>
                    InputManager.MoveTouchTo(
                        new Touch(
                            TouchSource.Touch1,
                            getTouchDownPos(TouchSource.Touch1) + new Vector2(70)
                        )
                    )
            );
        ...
        public void TestHoldTouchAndNudgeBeforeRightClick()
        {
            ...
            AddWaitStep("wait a bit", 4);
            AddAssert(
                "right click received at original position",
                () =>
                {
                    bool event1 =
                        primaryReceptor.MouseEvents.Dequeue() is MouseDownEvent down
                        && down.Button == MouseButton.Right
                        && down.ScreenSpaceMousePosition == getTouchDownPos(TouchSource.Touch1);
                    bool event2 =
                        primaryReceptor.MouseEvents.Dequeue() is MouseUpEvent up
                        && up.Button == MouseButton.Right;
                    return event1 && event2 && primaryReceptor.MouseEvents.Count == 0;
        ...
        public void TestHoldTwoTouchesAndReleaseFirstBeforeRightClick()
        {
            ...
            AddStep(
                "retrieve touch1 and touch2 receptor",
                () =>
                {
                    primaryReceptor = receptors[(int)TouchSource.Touch1];
            ...
            AddStep(
                ...
                () =>
                {
                    ...
                    primaryReceptor.HandleMouse = secondaryReceptor.HandleMouse = e =>
                        e is MouseButtonEvent button && button.Button == MouseButton.Right;
                }
            );
            ...
            AddStep(
                ...
                () =>
                    InputManager.BeginTouch(
                        new Touch(TouchSource.Touch2, getTouchDownPos(TouchSource.Touch2))
                    )
            );
            AddStep(
                "end first",
            ...
            AddWaitStep("wait a bit", 4);
            AddAssert(
                "no right click received",
                () =>
                    primaryReceptor.MouseEvents.Count == 0
                    && secondaryReceptor.MouseEvents.Count == 0
            );
        }
