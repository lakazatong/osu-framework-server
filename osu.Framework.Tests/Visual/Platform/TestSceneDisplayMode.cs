...
namespace osu.Framework.Tests.Visual.Platform
{
    [System.ComponentModel.Description(
        "For complete validation, run this with different window modes and resolutions at startup."
    )]
    ...
    public partial class TestSceneDisplayMode : FrameworkTestScene
    {
            ...
            Child = new BasicScrollContainer
            {
                ...
                Child = new FillFlowContainer
                {
                    ...
                    Children = new Drawable[]
                    {
                        ...
                        new SpriteText
                        {
                            Current = textConfigSizeFullscreen,
                            Font = FrameworkFont.Condensed,
                        },
                        new SpriteText
                        {
                            Current = textConfigWindowMode,
                            Font = FrameworkFont.Condensed,
                        },
                        new SpriteText
                        {
                        ...
                        new SpriteText
                        {
                            Current = textCurrentDisplay,
                            Font = FrameworkFont.Condensed,
                        },
                        new SpriteText
                        {
                            Current = textCurrentDisplayMode,
                            Font = FrameworkFont.Condensed.With(size: 16),
                        },
                        new SpriteText
                        {
                            Text = "Available display modes for current display:",
                            Font = FrameworkFont.Condensed,
                        },
                        textWindowDisplayModes = new TextFlowContainer(text =>
                            text.Font = FrameworkFont.Condensed.With(size: 16)
                        )
                        {
                            Width = 1000,
                            AutoSizeAxes = Axes.Y,
        ...
        protected override void LoadComplete()
        {
            ...
            configSizeFullscreen.BindValueChanged(
                e => textConfigSizeFullscreen.Value = $"SizeFullscreen: {e.NewValue}",
                true
            );
            configWindowMode.BindValueChanged(
                e => textConfigWindowMode.Value = $"WindowMode: {e.NewValue}",
                true
            );

            window.CurrentDisplayMode.BindValueChanged(
                e => textCurrentDisplayMode.Value = $"CurrentDisplayMode: {e.NewValue}",
                true
            );
            window.CurrentDisplayBindable.BindValueChanged(
                e =>
                    Schedule(() =>
                        textWindowDisplayModes.Text = string.Join('\n', e.NewValue.DisplayModes)
                    ),
                true
            );
        }
        ...
        public void TestDisplayModeSanity(WindowMode? windowMode)
        {
            switch (windowMode)
            {
                case null: // test startup
                    ...
                    checkDisplayModeSanity(
                        configWindowMode.Value == WindowMode.Fullscreen
                            && !configSizeFullscreen.IsDefault,
                        configWindowMode.Value != WindowMode.Windowed
                    );
                    break;
                ...
                case WindowMode.Borderless:
                    AddStep(
                        $"change to {windowMode}",
                        () => configWindowMode.Value = windowMode.Value
                    );
                ...
                case WindowMode.Fullscreen:
                    ...
                    setFullscreenResolution(
                        currentDisplay.FindDisplayMode(new Size(1920, 1080)).Size
                    );
                    checkDisplayModeSanity(true, true);

                    setFullscreenResolution(
                        currentDisplay.FindDisplayMode(new Size(1280, 720)).Size
                    );
                    checkDisplayModeSanity(true, true);
                    break;
            }
        ...
        private void checkDisplayModeSanity(
            bool checkResolutionAgainstFullscreen,
            bool checkClientSize
        )
        {
            AddAssert(
                "DisplayIndex matches display",
                () => displayMode.DisplayIndex == currentDisplay.Index
            );
            AddAssert(
                "display has current mode",
                () => currentDisplay.DisplayModes.Any(mode => mode == displayMode)
            );
            AddAssert("mode has valid RefreshRate", () => displayMode.RefreshRate != 0);

            if (checkResolutionAgainstFullscreen)
                AddAssert(
                    "Size matches config fullscreen resolution",
                    () => displayMode.Size == configSizeFullscreen.Value
                );
            ...
            if (!checkResolutionAgainstFullscreen)
                ...
                AddAssert(
                    "Size matches bindable display resolution",
                    () => displayMode.Size == currentDisplay.Bounds.Size
                );

            AddAssert(
                "Size matches actual display resolution",
                () =>
                    displayMode.Size
                    == window.Displays.ElementAt(displayMode.DisplayIndex).Bounds.Size
            );
        ...
        {
            AddStep(
                $"set fullscreen to {resolution.Width}x{resolution.Height}",
                () => configSizeFullscreen.Value = resolution
            );
