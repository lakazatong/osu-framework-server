...
namespace osu.Framework.SourceGeneration.Generators
{
    public abstract class AbstractIncrementalGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            IncrementalValuesProvider<IncrementalSyntaxTarget> syntaxTargets = context
                .SyntaxProvider.CreateSyntaxProvider(
                    (n, _) => isSyntaxTarget(n),
                    (ctx, _) =>
                        returnWithEvent(
                            new IncrementalSyntaxTarget(
                                (ClassDeclarationSyntax)ctx.Node,
                                ctx.SemanticModel
                            ),
                            EventDriver.OnSyntaxTargetCreated
                        )
                )
                ...
                .Select(
                    (t, _) =>
                        returnWithEvent(
                            t.WithSemanticTarget(CreateSemanticTarget),
                            EventDriver.OnSemanticTargetCreated
                        )
                );
            ...
            IncrementalValuesProvider<IncrementalSyntaxTarget> distinctSyntaxTargets = syntaxTargets
                .Collect()
                .SelectMany(
                    (targets, _) =>
                    {
                        ...
                        Dictionary<IncrementalSyntaxTarget, long> maxGenerationIds = new Dictionary<
                            IncrementalSyntaxTarget,
                            long
                        >(IncrementalSyntaxTarget.SyntaxNameComparer.DEFAULT);

                        foreach (var target in targets)
                        {
                            maxGenerationIds.TryGetValue(target, out long existingValue);
                            maxGenerationIds[target] = Math.Max(
                                existingValue,
                        ...
                        HashSet<IncrementalSyntaxTarget> result =
                            new HashSet<IncrementalSyntaxTarget>(
                                IncrementalSyntaxTarget.SyntaxNameComparer.DEFAULT
                            );

                        // Filter out the targets, preferring the most recent at all times.
                        foreach (
                            IncrementalSyntaxTarget t in targets.OrderByDescending(t =>
                                t.GenerationId
                            )
                        )
                            result.Add(t);
            ...
            context.RegisterImplementationSourceOutput(
                distinctSyntaxTargets.Select((t, _) => t.SemanticTarget!),
                emit
            );
        }

        protected abstract IncrementalSemanticTarget CreateSemanticTarget(
            ClassDeclarationSyntax node,
            SemanticModel semanticModel
        );

        protected abstract IncrementalSourceEmitter CreateSourceEmitter(
